...
echo "[build]"
make clean clean-tiny_vit >/dev/null 2>&1
DEBUG_FLAG=""
if [ "${QUANT_DEBUG}" -gt 0 ]; then DEBUG_FLAG=" -DQUANT_DEBUG"; fi
make train_quant_tiny_vit CFLAGS_EXTRA="-march=native -DMXINT_ROUNDING=1${DEBUG_FLAG}" -j 8
mkdir -p build
cp train_quant_tiny_vit build/train_quant_tiny_vit
      ...
      for b in "${BITS[@]}"; do
        ...
        for SETA in "${SETA_ARR[@]}"; do
          for SEED in "${SEEDS[@]}"; do
            ...
            sbatch -c "$CORE" "$JOB" "$LOSS" "$SETA" "$SEED" "$W" "$DX" "$ACT" "$WBLK" "$DXBLK" "$ABLK" "$INB" "$INBLK" "$ACC" "$CORE" "$HALVE" "$LR_SCHEDULE" "$LR_PATIENCE" "$SMOM" "$MBITS" "$STATS" "$BNBITS"
