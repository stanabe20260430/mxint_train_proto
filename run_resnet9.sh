...
make clean-resnet9
make train_quant_resnet9 CFLAGS_EXTRA="-march=native -DMXINT_ROUNDING=1" -j 16
...
for SETA in "${SETAS[@]}"; do
  OUTDIR="out/local/resnet9_w16_blk32_mom3/${LOSS}_seta${SETA}"
  mkdir -p "$OUTDIR"
  echo "=== resnet9 ${LOSS} seta=${SETA} seed=${SEED} ==="
  /usr/bin/time -v ./train_quant_resnet9 \
    ...
    --accum-mode-bn two_stage --accum-shift-bn 12 \
  ...
  rc=${PIPESTATUS[0]}
  if [ "$rc" -ne 0 ]; then
    echo "=== FAILED (rc=$rc): resnet9 ${LOSS} seta=${SETA} ===" | tee -a "$OUTDIR/seed${SEED}.log"
  fi
done
