...
make clean-vgg7
make train_quant_vgg7 CFLAGS_EXTRA="-march=native -DMXINT_ROUNDING=1" -j 16
...
for SETA in "${SETAS[@]}"; do
  OUTDIR="out/local/vgg7_w16_blk32_mom3/${LOSS}_seta${SETA}"
  mkdir -p "$OUTDIR"
  echo "=== vgg7 ${LOSS} seta=${SETA} seed=${SEED} ==="
  /usr/bin/time -v ./train_quant_vgg7 \
  ...
  rc=${PIPESTATUS[0]}
  if [ "$rc" -ne 0 ]; then
    echo "=== FAILED (rc=$rc): vgg7 ${LOSS} seta=${SETA} ===" | tee -a "$OUTDIR/seed${SEED}.log"
  fi
done
