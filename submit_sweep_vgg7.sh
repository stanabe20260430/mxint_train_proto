...
echo "[build]"
make clean clean-vgg7 >/dev/null 2>&1
DEBUG_FLAG=""
if [ "${QUANT_DEBUG}" -gt 0 ]; then DEBUG_FLAG=" -DQUANT_DEBUG"; fi
make train_quant_vgg7 CFLAGS_EXTRA="-march=native -DMXINT_ROUNDING=1${DEBUG_FLAG}" -j 8
mkdir -p build
cp train_quant_vgg7 build/train_quant_vgg7
      ...
      for b in "${BITS[@]}"; do
        ...
        for SETA in "${SETA_ARR[@]}"; do
          for SEED in "${SEEDS[@]}"; do
            ...
            echo "[submit] loss=${LOSS} seta=${SETA} seed=${SEED} bits=${W}/${DX}/${ACT}/in${INB} blocks=${WBLK}/${DXBLK}/${ABLK}/in${INBLK} acc=${ACC} mom=${SMOM}/${MBITS} stats=${STATS} sched=${LR_SCHEDULE}/${LR_PATIENCE}"
            sbatch -c "$CORE" "$JOB" "$LOSS" "$SETA" "$SEED" "$W" "$DX" "$ACT" "$WBLK" "$DXBLK" "$ABLK" "$INB" "$INBLK" "$ACC" "$CORE" "$HALVE" "$LR_SCHEDULE" "$LR_PATIENCE" "$SMOM" "$MBITS" "$STATS"
